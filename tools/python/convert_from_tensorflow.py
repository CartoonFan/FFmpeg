...
class Operand(object):
    ...
    def __init__(self, name, dtype, dims):
        ...
        self.iotype2str = {Operand.IOTYPE_INPUT: 'in',
                           Operand.IOTYPE_OUTPUT: 'out', Operand.IOTYPE_INTERMEDIATE: 'inout'}
        self.dtype2str = {Operand.DTYPE_FLOAT: 'DT_FLOAT',
    ...
    def __str__(self):
        return "{}: (name: {}, iotype: {}, dtype: {}, dims: ({},{},{},{}) used_count: {})".format(self.index,
                                                                                                  self.name, self.iotype2str[
                                                                                                      self.iotype], self.dtype2str[self.dtype],
...
class TFConverter:
    def __init__(self, graph_def, nodes, outfile, dump4tb):
        ...
        self.name_node_dict = {}
        self.edges = {}
        self.conv_activations = {'Relu': 0, 'Tanh': 1,
                                 'Sigmoid': 2, 'None': 3, 'LeakyRelu': 4}
    ...
    def dump_complex_conv2d_to_file(self, node, f):
        ...
        has_bias = 1
        np.array([self.op2code[node.op], dilation, padding, self.conv_activations[activation],
        ...
        input_operand_index = self.add_operand(
            input_name, Operand.IOTYPE_INPUT)

        if anode is not None:
            output_operand_index = self.add_operand(
                anode.name, Operand.IOTYPE_OUTPUT)
        else:
            output_operand_index = self.add_operand(
    ...
    def dump_simple_conv2d_to_file(self, node, f):
        ...
        input_operand_index = self.add_operand(
            input_name, Operand.IOTYPE_INPUT)
        output_operand_index = self.add_operand(
            node.name, Operand.IOTYPE_OUTPUT)
        np.array([input_operand_index, output_operand_index],
                 dtype=np.uint32).tofile(f)
    ...
    def dump_depth2space_to_file(self, node, f):
        ...
        self.converted_nodes.add(node.name)
        input_operand_index = self.add_operand(
            node.input[0], Operand.IOTYPE_INPUT)
        output_operand_index = self.add_operand(
            node.name, Operand.IOTYPE_OUTPUT)
        np.array([input_operand_index, output_operand_index],
                 dtype=np.uint32).tofile(f)
    ...
    def dump_mathbinary_to_file(self, node, f):
        ...
        np.array([self.op2code['MathBinary'],
                  self.mathbin2code[node.op]], dtype=np.uint32).tofile(f)
        if i0_node.op == 'Const':
            ...
            input_operand_index = self.add_operand(
                i1_node.name, Operand.IOTYPE_INPUT)
            ...
            input_operand_index = self.add_operand(
                i0_node.name, Operand.IOTYPE_INPUT)
            np.array([input_operand_index], dtype=np.uint32).tofile(f)
    ...
    def dump_operands_to_file(self, f):
        ...
        for operand in operands:
            ...
            np.array([operand.iotype, operand.dtype],
                     dtype=np.uint32).tofile(f)
            np.array([operand.dims[0], operand.dims[1], operand.dims[2],
                      operand.dims[3]], dtype=np.uint32).tofile(f)
